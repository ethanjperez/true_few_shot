...
class RTEReader(object):
    ...
    def _get_file(self, split):
        ...
        if split.lower() == "train":
            file = os.path.join("data", "fewglue", "RTE", f"train.seed-{self.config.train_set_seed}.jsonl")
    ...
    def prepare_pet_batch(self, batch, mode="PET1"):
        ...
        return remove_extra_padding(list_input_ids).to(device), torch.tensor(list_mask_idx).to(device), label
    ...
    def prepare_eval_pet_mlm_batch(self, batch, mode="PET1"):
        ...
        return remove_extra_padding(list_input_ids).to(device), remove_extra_padding(list_masked_input_ids).to(device)
